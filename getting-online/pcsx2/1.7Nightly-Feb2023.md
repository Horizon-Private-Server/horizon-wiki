...
_If you find that this guide is out of date, please let us know in the [Horizon Discord](https://discord.gg/horizonps) and we will try to update it when we have time. Last verified on 2 July 2023._    
...
7. Visit [this link](https://nmap.org/download) and find the "Latest stable release" of Nmap for your operating system. Download and install it.  
...
- [Alternate Network Setup (Sockets)](/getting-online/pcsx2/1.7Nightly-Feb2023.md#alternate-network-setup-sockets)
...
For some titles, the Nmap/PCAP setup will work without any further steps. For others, extra steps are required. Requirements can vary on a game-by-game basis. Here are some helpful resources for checking the requirements of each title:  
...
If the Nmap/PCAP network setup detailed above isn't working, you may want to try this alternate setup for Horizon-supported titles only. **As of July 2023, PCSX2 updates have caused issues with this setup method. We now recommend the Nmap/PCAP setup above.** To use this alternate setup, substitute these steps instead of the corresponding step number above:  

7. (alternate) Download the [internal DNS file here](/assets/cheats/pcsx2_template_hosts.ini) (visit the link, right click the "Raw" button near the top right, then "Save As"). Save it into your PCSX2 folder.  
...
The other part that is different is the correct DNS address to enter as the 'Primary DNS' in step 12 above. On the screen where you enter the DNS server address, enter the DNS: `192.0.2.1` for the 'Primary DNS', matching what is pointed out by the yellow arrow above. The 'Secondary DNS' should be left blank.
> Why enter `192.0.2.1` for the Primary DNS address and not one of the options listed in the `#how-to-play` channel of the Horizon Discord server? Using `192.0.2.1` allows access to the internal DNS addresses you downloaded and imported for the alternate setup above. In non-technical terms, it is like a compass that points you to the correct DNS depending on which version of the game you have. **`192.0.2.1` does NOT apply for PS2 players, or PCSX2 players using PCAP.**.  
