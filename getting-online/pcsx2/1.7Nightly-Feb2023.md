...
If NEITHER the Sockets/Auto or Nmap/PCAP methods are working, there is a third network setup method you can try. **This method may work for Horizon-supported titles only.**  
...
Then, further down the same window next to "DNS1 Address" change the drop down selection to "Internal".  

Then, click on the "Internal DNS" tab under the Ethernet Device selection. Click the "Import" button and navigate to the hosts.ini file that you saved. Click OK on the next window to add all of the hosts in the file.  
...
#### Some other things that you can try:  
   - If the Sockets/Auto method is not working, try the Nmap/PCAP method - or vice versa. If neither is working, try the Sockets/Auto with internal DNS method.  
